...
import os
import string
from string_with_arrows import string_with_arrows as arrow_str
...
class Error:
	...
	def as_string(self) -> str:
		...
		result += f'File: {self.pos_start.file}, Line: {self.pos_start.ln + 1}'
		result += '\n\n' + arrow_str(self.pos_start.contents, self.pos_start, self.pos_end)
		return result
...
class RTError(Error):
	...
	def generate_traceback(self) -> str:
		...
		while ctx:
			result = f'  File: {pos.file}, Line: {str(pos.ln + 1)}, in {ctx.display_name}\n' + result
...
class Position:
	def __init__(self, idx, ln, col, file_, contents) -> None:
		self.idx = idx
		self.ln = ln
		self.col = col
		self.file = file_
	...
	def copy(self):
		return Position(self.idx, self.ln, self.col, self.file, self.contents)
...
class Lexer:
	def __init__(self, file_, text) -> None:
		self.file = file_
		self.text = text
		self.pos = Position(-1, 0, -1, file_, text)
	...
	def make_tokens(self) -> tuple[list, Error]:
		tokens = []
...
class ForNode:
	def __init__(self, var_name_tok, start_value_node, end_value_node, increment_value_node, body_node) -> None:
		self.var_name_tok = var_name_tok
	...
	def subbed_by(self, other):
		if isinstance(other, Number):
			...
			try:
				...
				return None, RTError(
					other.pos_start, other.pos_end,
					f"Element at index '{other.value}' is out of bounds",
					self.context
				)
		else:
			return None, Value.illegal_operation(self, other)
	...
	def check_args(self, arg_names, args):
		...
		if len(args) > len(arg_names):
			return res.failure(RTError(
				...
				f"Function '{self.name}', expected {len(arg_names)} arguments, found {len(args)}",
				self.context
			))
...
class BuiltInFunction(BaseFunction):
	...
	def execute_input_int(self, exec_ctx):
		while True:
			...
			try:
				...
				print(f"'{text}' must be an integer. Use print instead for strings")
	...
	def execute_pop(self, exec_ctx):
		...
		try:
			...
			return RTResult().failure(RTError(
				self.pos_start, self.pos_end,
				f"Element at index '{index.value}' is out of bounds",
				exec_ctx
			))
...
class Interpreter:
	...
	def visit_VarAccessNode(self, node, context):
		...
		if not value:
			return res.failure(RTError(
				...
				f"'{var_name}' is not defined in the current scope",
				context
			))
	...
	def visit_ForNode(self, node, context):
		...
		if node.increment_value_node:
			increment_value = res.register(self.visit(node.increment_value_node, context))
			if res.error: return res
		else:
